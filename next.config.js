    ...
    return [
      {
        source: '/:path*',
        headers: [
          { key: 'Content-Security-Policy', value: String(CSP).split(';').map((d) => d.trim()).filter((d) => d && !d.startsWith('frame-ancestors')).concat('frame-ancestors ' + __embed.frameAncestors({ brandedDomain: 'javarispirits.com' })).join('; ') },
